...
const allocateMemory = (sizeInMB) => {
  ...
  try {
    ...
    console.log(`Allocated ~${sizeInMB}MB of memory, total chunks: ${memoryUsage.value.length}`)
    return true
  } catch (err) {
      ...
      try {
        ...
        if (stressLevel.value > 2) {
          ...
          for (let c = 0; c < copies; c++) {
            ...
            const redundantCopy = await createThumbnailFromImageSrc(imageSrc, true)
        ...
        if (stressLevel.value > 1) {
          await new Promise(resolve => setTimeout(resolve, 100 * stressLevel.value))
        }
      } catch (err) {
  ...
  <div class="page-container">
    <div class="card">
        ...
        <div class="flex justify-between items-center">
          ...
          <div class="stress-controls">
            <div class="flex items-center space-x-4">
              <div>
                <label for="stress-level" class="block text-sm font-medium">Browser Stress Level:</label>
                <div class="flex items-center space-x-2">
                  <input 
                    ...
                    class="stress-slider"
                  />
                  <span class="text-sm font-bold" :class="{'text-red-600': stressLevel > 3}">
                    {{ stressLevel }}/5
                  </span>
                </div>
              </div>
              <div>
                <label for="memory-size" class="block text-sm font-medium">Memory Allocation (MB):</label>
                <div class="flex items-center space-x-2">
                  <input 
                    type="number" 
                    id="memory-size" 
                    ...
                    class="memory-input"
                  />
                  <button @click="startStressTest" class="stress-button" :disabled="cpuIntensiveTasksRunning">
                    {{ cpuIntensiveTasksRunning ? 'Running...' : 'Stress Test' }}
                  </button>
                </div>
              </div>
            </div>
            <div class="text-xs text-gray-500 mt-1">
              <span :class="{'text-red-500': memoryUsage.length > 10}">Memory chunks: {{ memoryUsage.length }}</span>
              <span v-if="stressLevel > 3" class="ml-2 text-red-500 font-bold">Warning: High stress levels may crash your browser!</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="thumbnails.length > 0" class="max-w-screen-2xl mx-auto w-full px-6">
            ...
            >
              <div class="thumbnail-header">
                ...
                <div class="flex justify-between">
                  ...
                  <p v-if="thumb.fileSize" class="thumbnail-filesize">
                    {{ thumb.fileSize }}
                  </p>
                </div>
              </div>
        ...
        <div v-if="totalProcessingTime && !isProcessing" class="processing-stats-container">
